...
def safe_for_synth(csv_fpath, code_dir):
    output_csv = str(csv_fpath).replace(".csv", "_synthseg_safe_results.csv")
    ...
    NV_labels[2] = labels[2]
    NV_labels[3:9] = labels[5:11]
    NV_labels[9:16] = labels[13:20]
    NV_labels[16:] = labels[22:]

    NV_subjects = np.empty((r_rows-1,r_cols-6), dtype=object)
    NV_subjects[:,0:2] = subjects[:,0:2]
    NV_subjects[:, 2] = subjects[:, 2]
    NV_subjects[:,3:9] = subjects[:,5:11] 
    NV_subjects[:,9:16] = subjects[:,13:20] 
    NV_subjects[:,16:] = subjects[:,22:]
    ...
    mu_std_norm = np.genfromtxt(code_dir/"NV_occup_synth_norm_vol.csv", delimiter=',', dtype=None, encoding='utf-8')
    mu_std_vol = np.genfromtxt(code_dir/"NV_synth_norm_vol.csv", delimiter=',', dtype=None, encoding='utf-8')
    ...
    def summarize_stats(mu_std_norm, mu_std_vol, data_norm, data_vol):
        ...
        for i in range(rows-1):
            ...
            for j in range(cols-1):
                ...
                offset = round((curr_norm - mu_norm)/std_norm,6) 
    ...
    occ_95, occ_99, zeros, vol_95, vol_99 = summarize_stats(mu_std_norm, mu_std_vol, NV_mod_data, vol_data) 
    ...
    print(f"Safe decisions for synthseg saved in {output_csv}")